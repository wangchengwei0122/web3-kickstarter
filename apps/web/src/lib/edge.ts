...
import deployment from "../../../../packages/contracts/deployments/<CHAIN_ID>.json";
...
async function fetchFromEdge(cursor: number, limit: number, sort: "latest" | "deadline"): Promise<CampaignPage> {
...
async function fetchDirectOnChain(limit: number, cursor: number): Promise<CampaignPage> {
  const rpcUrl = resolveEnv("NEXT_PUBLIC_RPC_URL");
  ...
  const deployBlock = parseBig(resolveEnv("NEXT_PUBLIC_DEPLOY_BLOCK") ?? manifest.deployBlock);
